#!/usr/bin/env python3
"""
Script para crear evaluaciones del primer trimestre 2023
...
def crear_gestion_primer_trimestre():
    """Crea la gestión del primer trimestre 2023 si no existe"""
    app = create_app()
    
    with app.app_context():
        # Verificar si ya existe la gestión
        gestion = Gestion.query.filter_by(anio=2023, periodo="Primer Trimestre").first()
        
        if gestion:
            print(f"AVISO: Gestión 2023 Primer Trimestre ya existe (ID: {gestion.id})")
            return gestion.id
        ...
        try:
            ...
            print(f"SUCESO: Gestión 2023 Primer Trimestre creada (ID: {result['id']})")
            return result['id']
...
def crear_evaluaciones_primer_trimestre():
    """Crea evaluaciones para el primer trimestre de 2023"""
    print("CREANDO EVALUACIONES PRIMER TRIMESTRE 2023")
    print("=" * 60)
    ...
    gestion_id = crear_gestion_primer_trimestre()
    if not gestion_id:
        print("ERROR: No se pudo obtener/crear la gestión del primer trimestre 2023")
        return
    ...
    with app.app_context():
        try:
            ...
            if evaluaciones_existentes > 0:
                print(f"AVISO: Ya existen {evaluaciones_existentes} evaluaciones para el primer trimestre 2023")
            ...
            config_evaluaciones = {
                ...
                'Examenes': {'cantidad': 2, 'min': 14, 'max': 35},
                'Tareas': {'cantidad': 4, 'min': 21, 'max': 35},
                ...
                for materia_curso in materias_curso:
                    ...
                    for tipo_nombre, config in config_evaluaciones.items():
                        ...
                        for i in range(config['cantidad']):
                            ...
                            # Crear evaluación
                            evaluacion_data = {
                                "descripcion": f"{tipo_nombre} #{i+1} - 2023 T1",
            ...
            print("\nPRIMER TRIMESTRE 2023 COMPLETADO!")
...
if __name__ == "__main__":
    crear_evaluaciones_primer_trimestre()
